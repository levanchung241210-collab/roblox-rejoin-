...
# ==================== SỬA ĐÚNG LỖI VÒNG LẶP SPAM OPEN APP ====================
resume_account() {
    local acc=$1
    write_state "$acc" "STATE" "RUNNING"
    write_state "$acc" "PAUSED_TIME" "0"
    write_state "$acc" "UPTIME_START" "$(date +%s)" # Reset lại thời gian để không bị check crash ngay lập tức
...
monitor_account() {
    ...
    # SỬA LỖI CHÍ MẠNG: Nếu không tìm thấy PID (game bị tắt/đang khởi động)
    if [ -z "$pid" ]; then
        # Chỉ khi thời gian trôi qua vượt quá LOAD_TIME (180s) mới coi là Crash thật sự và kích hoạt restart
        if [ $elapsed -ge $LOAD_TIME ]; then
            log_msg "CRASH" "Process crashed - Auto rejoin" "$acc"
            smart_restart "$acc" "CRASH"
        else
            # Nếu chưa đủ 180s, tức là app đang trong quá trình túc tắc mở lên, KHÔNG SPAM LỆNH NỮA
            log_msg "INFO" "Game đang trong thời gian khởi động, đợi thêm... ($elapsed/$LOAD_TIME s)" "$acc"
        fi
        return 0
    fi
    
    # Nếu game có PID (đang chạy) nhưng thời gian uptime chưa vượt quá thời gian load màn hình ban đầu, tạm bỏ qua để tránh quét lỗi sớm
